...
namespace OwlGame.Entities
{
    ...
    //Date: 23/10/18
    //Description: This class is used to read animation files on certain class entities where its specified EntityName : AnimatedSpriteEntity 
    ...
    public class AnimatedSpriteEntity : CCNode
    {
        ...
        void CreateSprite()
        {
            ...
            sprite.TextureRectInPixels = new CCRect(1258, 0, 100, 100);
            sprite.ContentSize = new CCSize(100, 100);

        }

        protected void LoadAnimations(string fileName) //reads animations from XML Files (.achx in animations folder)
        {
